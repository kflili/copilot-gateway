...
def _wsl_rc_block_fish(port: int, wsl_userprofile: str | None = None) -> str:
    ...
    if wsl_userprofile:
        ...
        wslconfig_pattern = f"if test -e {wslconfig_path}"
        wslconfig_grep_arg = wslconfig_path
    else:
        # Fish errors on a glob that matches nothing — guard with `set` and
        # `count` so we only call grep when the path resolves to >=1 file.
        wslconfig_pattern = ("set -l _cg_cfgs /mnt/c/Users/*/.wslconfig 2>/dev/null; "
                             "if test (count $_cg_cfgs) -gt 0")
        wslconfig_grep_arg = "$_cg_cfgs"
    return f"""{WSL_RC_BLOCK_MARKER}
# Auto-generated by copilot-gateway tray_app.py — do not edit by hand.
# Resolves Windows host IP at every shell start for use with the gateway.
function _copilot_gateway_resolve_host
    # 1. WSL2 mirrored mode: gateway is reachable at 127.0.0.1. Fish errors
    #    on a glob that matches nothing — guard via `set` + `count` before
    #    invoking grep, otherwise `~/.wslconfig` (no file) breaks shell start.
    {wslconfig_pattern}
        if grep -lE 'networkingMode[[:space:]]*=[[:space:]]*mirrored' {wslconfig_grep_arg} 2>/dev/null >/dev/null
            echo 127.0.0.1; return 0
        end
    end
    # 2. host.docker.internal (if user enabled it in .wslconfig)
